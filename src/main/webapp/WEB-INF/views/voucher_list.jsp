...
<head>
    <meta charset="UTF-8">
    <title>Quản lý voucher - Milk Tea Shop</title>
    ...
    <div class="main-content">
        <div class="container">
            ...
            <div class="search-bar d-flex justify-content-between align-items-center">
                ...
                <a href="${pageContext.request.contextPath}/voucher/add" class="btn-add">
                ...
                <table class="table">
                    ...
                    <tbody>
                        <c:forEach var="voucher" items="${vouchers}">
                            <tr>
                                <td><div class="product-name">${voucher.code}</div></td>
                                <td><div class="product-name">${voucher.name}</div></td>
                                <td><div class="product-name"><fmt:formatDate value="${voucher.dateStartAsDate}" pattern="HH:mm:ss dd/MM/yyyy"/></div></td>
                                <td><div class="product-name"><fmt:formatDate value="${voucher.dateEndAsDate}" pattern="HH:mm:ss dd/MM/yyyy"/></div></td>
                                <c:if test="${voucher.percentDiscount != null}">
                                    <td><div class="product-name">${voucher.percentDiscount}%</div></td>
                                </c:if>
                                <c:if test="${voucher.valueDiscount != null}">
                                    <td><div class="product-name">${voucher.valueDiscount}</div></td>
                                </c:if>
                                <td><span class="voucher-remaining" data-end="${voucher.dateEndAsDate.time}"></span></td>
                                <td>
                                    <c:if test="${permission == 'khách hàng'}">
                                        <button class="btn btn-edit btn-sm btn-copy-voucher" data-code="${voucher.code}">
                                            <i class="fas fa-copy"></i> Sao chép mã Voucher
                                        </button>
                                    </c:if>
                                </td>
                                <c:if test="${permission != 'khách hàng'}">
                                    <td>
                                        <a href="${pageContext.request.contextPath}/voucher/edit/${voucher.code}" class="btn-action btn-edit">
                                            <i class="fas fa-edit me-1"></i>Sửa
                                        </a>
                                        <a href="${pageContext.request.contextPath}/voucher/delete/${voucher.code}" class="btn-action btn-delete" onclick="return confirm('Bạn có chắc chắn muốn xóa voucher này?');">
...
<jsp:include page="/WEB-INF/views/common/footer.jsp" />
    ...
    function formatRemaining(ms) {
        if (ms <= 0) return '<span class="text-danger fw-bold">Hết hạn</span>';
    ...
    function updateVoucherRemaining() {
        const now = Date.now();
        document.querySelectorAll('.voucher-remaining').forEach(span => {
            const end = parseInt(span.getAttribute('data-end'));
            span.innerHTML = formatRemaining(end - now);
        });
    }
    updateVoucherRemaining();
    setInterval(updateVoucherRemaining, 1000);
    </script>
    <script>
    // Copy voucher code
    document.addEventListener('DOMContentLoaded', function() {
      document.querySelectorAll('.btn-copy-voucher').forEach(btn => {
        btn.addEventListener('click', function() {
          const code = btn.getAttribute('data-code');
          navigator.clipboard.writeText(code).then(() => {
            showToast('Mã đã được sao chép');
          });
        });
      });
    });
